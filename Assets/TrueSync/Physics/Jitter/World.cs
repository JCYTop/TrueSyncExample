    ...
    public class World : IWorld
    {
        ...
        private void UpdateArbiterContacts(Arbiter arbiter)
        {
            ...
            for (int i = arbiter.contactList.Count - 1; i >= 0; i--)
            {
                ...
                {
                    TSVector diff;
                    TSVector.Subtract(ref c.p1, ref c.p2, out diff);
        ...
        private void IntegrateCallback(object obj)
        {
            ...
            if (!(body.isParticle))
            {
                ...
                if (angle < FP.EN3)
                {
                    ...
                    TSVector.Multiply(ref body.angularVelocity, (FP.Half * timestep - (timestep * timestep * timestep) * (2082 * FP.EN6) * angle * angle),
                        out axis);
                }
                else
                {
        ...
        internal bool CanBodiesCollide(RigidBody body1, RigidBody body2)
        {
            ...
            {
                if ((body1.IsColliderOnly && body1.IsStaticNonKinematic && body2.IsStaticNonKinematic) ||
                    (body2.IsColliderOnly && body2.IsStaticNonKinematic && body1.IsStaticNonKinematic))
                {
                    return false;
                }
            }
